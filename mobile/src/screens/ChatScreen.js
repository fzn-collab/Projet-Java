...
import { getMessages, sendMessage, markAsRead } from '../services/apiService';
...
export default function ChatScreen({ route }) {
  ...
  const handleSend = async () => {
    ...
    setSending(true);
    try {
      const newMessage = await sendMessage(conversationId, currentUserId, otherUserId, inputText.trim());
        ...
        <FlatList
          ...
          ListEmptyComponent={
            <View style={styles.emptyContainer}>
              ...
              <Text style={styles.emptySubtitle}>Envoyez votre premier message à {otherUserId}</Text>
            </View>
          }
...
const styles = StyleSheet.create({
  ...
  onlineIndicator: {
    ...
    backgroundColor: '#4CAF50', borderWidth: 2, borderColor: '#1A73E8',
  },
  headerInfo: { flex: 1 },
  ...
  // Date separator
  dateSeparator: { flexDirection: 'row', alignItems: 'center', marginVertical: 15, paddingHorizontal: 10 },
  dateLine: { flex: 1, height: 1, backgroundColor: '#DDD' },
  dateText: { marginHorizontal: 10, fontSize: 12, color: '#888', fontWeight: '500' },
